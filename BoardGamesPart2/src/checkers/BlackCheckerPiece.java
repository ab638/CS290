...
public class BlackCheckerPiece extends CheckerPiece {

    public BlackCheckerPiece(Position position, Colors content, boolean isKing) {
    ...
    public Vector<CheckerMove> addMoves(CheckerBoard board) {
        ...
        Colors cellContent = board.board[from.getX()][from.getY()].getColors();

        if (cellContent == Colors.UNDER) {
            return moves;
        }
        if (cellContent == Colors.BLACK) {
